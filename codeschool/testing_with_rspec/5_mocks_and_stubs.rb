...
# Named Stubs
# It's possible to give a stub a name by passing it in as the first parameter
# to the stub method. Name your zombie and tweet stubs :zombie and :tweet.
...
# Mocking
# Update the spec so that whenever a tweet is created, we verify
# that email_tweeter is called on the tweet object.
...
# We've changed the code below so we're mocking ZombieMailer.tweet method
# instead of the entire method. There is still more to do to make this
# example work. First, finish the let(:mail) statement below by creating 
# a stub with a deliver method that returns true. Then update the mock 
# so the call to the tweet method returns the mail stub you created.
...
# We know that the ZombieMailer is being called now, but we aren't
# verifying the parameters. Update the spec to verify that the zombie
# and tweet are getting passed as parameters into the tweet method.
...
# Verifying Delivery
# We've verified that the ZombieMailer is being called with the correct
# parameters, but not that deliver is being called on the resulting mail
# object. Complete the new example by stubbing out the ZombieMailer.tweet
# method and having it return mail. Then make sure that the deliver
# method is called on the mail object.
...
# Message Counts
# Notice on the tweet.rb source that we're now emailing every zombie
# in the horde in our email_tweeter. Update the spec to verify that tweet
# is called exactly as many times as there are zombies.
